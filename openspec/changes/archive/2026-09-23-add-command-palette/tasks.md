...
- [x] 1.3 Add `CommandRegistry` with `available(ctx)` (`isAvailable` filter; role gating deferred until #178 lands) and `search(query, ctx)`, and verify with unit tests that a command whose own check fails is excluded and that an empty query returns every available command in group order
