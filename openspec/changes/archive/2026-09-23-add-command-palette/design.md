...
CommandRegistry
  ...
  available(ctx)          isAvailable filter
  search(query, ctx)      available, then fuzzy-filtered and ranked
```

Role-gated commands (e.g. an admin-only "delete") are deferred until an identity provider lands
with #178; `available(ctx)` filters on `isAvailable` alone for now.
