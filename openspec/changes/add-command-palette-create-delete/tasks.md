...
- [x] 2.1 Add `CommandGroup.create` and the five "New …" commands requiring `Roles.editor`, which go to the kind's list after a successful create unless already there. Verify with unit tests for role filtering and with a widget test that "New item" on `/fonts` opens the item dialog, and that submitting lands on `/items` while cancelling stays on `/fonts`
- [x] 2.2 Append a role-checked "Delete…" child to every entity (tabs first where there are any; attributes and notifications get only this child). Verify with unit tests that an admin sees it for all five kinds, an editor sees none, and a session without a provider sees it; and with a widget test that choosing it opens `ModelDeleteDialog` for that entity and cancelling deletes nothing
- [x] 2.3 Add the "Delete this …" commands for items, fonts, sounds and notifications, requiring `Roles.admin` and available only on that kind's detail page with a selection. Verify with unit tests for availability, and with a widget test that confirming by typing the name removes the item (fake HTTP) and returns to `/items`
