...
- Role-gating create and delete (an editor role for creating, an admin role for deleting) is
  deferred until an identity provider lands with #178; nothing in the app is role-gated today.
...
- **Create commands**: five registry commands in a new `CommandGroup.create`, which sits after
  Navigation in group order. Role-gating is deferred until #178 lands. `run` reads the project key
  from the store, awaits `openModelCreateDialog`, and on `true` goes to the kind's list unless
  already there.
- **Delete children**:
  ...
  - The child is always added for now; role-gating is deferred until #178 lands.
- **"Delete this …" commands**: registry commands per kind with a detail page. Role-gating is
  deferred until #178 lands; they are available when the location is that kind's detail route and
  the selection is set. They call `confirmAndDeleteModel` with `returnToList: true`.
