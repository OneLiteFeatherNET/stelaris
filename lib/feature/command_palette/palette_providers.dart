...
class EntityProvider implements PaletteProvider {
  ...
  EntityResults resolveEntities(
    ...
    final bool mayDelete = context.state.auth.hasRole(Roles.admin);
    final List<StelarisCommand> entries = [
      for (final _Candidate candidate in shown)
        ?_commandFor(candidate, context.state, mayDelete),
    ];
  ...
  StelarisCommand? _commandFor(
    ...
    List<StelarisCommand> actionsFor<E extends DataModel>(
      E model,
      DeleteSpec<E> spec,
    ) => [if (mayDelete) _deleteChild(kind, model, candidate.name, spec)];
