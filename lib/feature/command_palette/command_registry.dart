...
class CommandRegistry {
  ...
  /// The commands [context] allows: the session holds the required role, if
  /// any, and the command's own availability check passes.
  ///
  /// `AuthState.hasRole` already grants every role when no identity provider
  /// is configured, so a deployment without one sees every command.
  List<StelarisCommand> available(CommandContext context) {
    return all.where((command) {
      final String? role = command.requiredRole;
      if (role != null && !context.state.auth.hasRole(role)) {
        return false;
      }
